...
namespace LuaTinker.StackHelpers
{
	extension StackHelper
	{
		...
		public static T Pop<T>(Lua lua, int32 index) where T : var, struct*
		{
			...
			{
				let tinkerState = lua.TinkerState;
				TryThrowErrorF(lua, tinkerState, (e) => e($"can't convert argument {index} to 'ptr {GetBestLuaClassName<T>(tinkerState, .. scope .())}'"));
				return default;
			}
		}
		...
		public static ref T Pop<T>(Lua lua, int32 index) where T : var, struct
		{
			...
			{
				let tinkerState = lua.TinkerState;
				//TryThrowErrorF(lua, tinkerState, (e) => e($"can't convert argument {index} to '{GetBestLuaClassName<T>(tinkerState, .. scope .())}'"));
				return ref dummy;
			}
			...
			if (result != .OkNoMetaTable)
			{
				...
				if (ptr == null)
				{
					TryThrowError(lua, "null pointer dereference");
					return ref dummy;
				}
		...
		public static ref T PopRef<T>(Lua lua, int32 index) where T : var
		{
			...
			{
				let tinkerState = lua.TinkerState;
				//TryThrowErrorF(lua, tinkerState, (e) => e($"can't convert argument {index} to 'ref {GetBestLuaClassName<T>(tinkerState, .. scope .())}'"));
				return ref dummy;
			}
		}
	}
